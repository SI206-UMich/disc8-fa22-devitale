...
def getLink(soup):
    tag = soup.find(title = "List of American Universities with Olympic medals")
    link = tag.get('href')
...
def main():
    ...
    soup = BeautifulSoup(requests.get('https://en.wikipedia.org/wiki/University_of_').text, 'html.parser')
